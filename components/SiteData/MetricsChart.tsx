...
export function MetricsChart({ siteId, dateRange, selectedMetrics }: MetricsChartProps) {
  ...
  const generateDateRange = (from: Date, to: Date, interval: 'hour' | 'day' | 'month'): { key: string; date: Date; displayDate: string }[] => {
    ...
    while (current <= to) {
      ...
      if (interval === 'hour') {
        ...
        displayDate = `${formatDateDisplay(current)} ${current.getHours()}:00`;
        ...
        const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        displayDate = `${months[current.getMonth()]} ${current.getFullYear()}`;
        ...
        sessions.forEach((session) => {
          ...
          if (session.created_at && session.last_seen) {
            const duration = Math.round(
              (new Date(session.last_seen).getTime() - new Date(session.created_at).getTime()) / 1000
            );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Metrics Over Time</CardTitle>
        <CardDescription>
          Click on metric cards above to add/remove them from the chart
        </CardDescription>
      </CardHeader>
      <CardContent>
        <ChartContainer
          ...
          <AreaChart data={chartData}>
            <defs>
              {selectedMetrics.map((metric) => (
                <linearGradient key={`fill${metric}`} id={`fill${metric}`} x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
            ...
            {selectedMetrics.map((metric, index) => (
              <Area
                ...
                strokeWidth={2}
                stackId={metric === "bounceRate" || metric === "avgDuration" ? metric : "a"}
              />
            ))}
