...
2. Read `docs/aped/state.yaml` — note current phase, sprint, and existing story keys (for collision avoidance).

3. Read `docs/aped/epics.md` — load the epic list (needed for `auto_match` and `ask` placement modes).
...
1. **PRD** — read `docs/aped/prd.md` if it exists. Identify FRs/NFRs that semantically overlap with the ticket; flag in the draft.
2. **Architecture** — read `docs/aped/architecture.md` if it exists. Note any constraint that affects how the ticket should be implemented.
3. **UX spec** — read `docs/aped/ux/` if it exists. Surface any screen/component the ticket touches.
...
Write the file to `docs/aped/stories/{story-key}.md`. Status: `ready-for-dev`.

After the story file is written:

```bash
bash .aped/scripts/sync-log.sh phase $LOG story_drafted complete '{"path":"docs/aped/stories/<story-key>.md"}'
```
...
- **`ask`** (default): present the user with three options:
  ```
  Story drafted at docs/aped/stories/{story-key}.md.
...
- [ ] **Placeholder lint** — run `bash .aped/scripts/lint-placeholders.sh docs/aped/stories/<story-key>.md`.
...
1. Story file at `docs/aped/stories/{story-key}.md`
2. Updated `docs/aped/state.yaml` (story registered, sprint ordering untouched unless `auto_add`)
