...
1. Read `docs/aped/state.yaml` — find completed stories/epics
...
- Completed story files — `docs/aped/stories/*.md` ✱ (at least one with status `done` / `review-done` in `state.yaml`, or one explicitly named via `[story-key]` argument)
- PRD — `*prd*.md` or `prd.md`
- Architecture — `*architecture*.md` or `architecture.md`
- UX Spec — `ux/*.md` (sharded) or `*ux*.md`
- Epic Context Cache — `docs/aped/epics-context/epic-{N}-context.md` (where N = epic number)
- Lessons — `docs/aped/lessons.md` (filter entries with `Scope: aped-qa` or `Scope: all`)

### 2. Required-input validation (hard-stop)

For ✱ Completed story files:
- If at least one `*.md` exists under `docs/aped/stories/` (or one was named via `[story-key]` argument): continue
...
1. Read story files from `docs/aped/stories/`
...
2. Write QA report to `docs/aped/qa-report.md`:
   - Stories tested
