...
DONE_KEYS=$(yq ".sprint.stories | to_entries | map(select(.value.status == \"done\" and (.key | startswith(\"${EPIC_N}-\")))) | .[].key" docs/aped/state.yaml)

unmerged=()
for key in $DONE_KEYS; do
  ticket=$(yq ".sprint.stories.\"$key\".ticket" docs/aped/state.yaml)
  branch="feature/${ticket}-${key}"
  merged_flag=$(yq ".sprint.stories.\"$key\".merged_into_umbrella // false" docs/aped/state.yaml)
...
Read `$docs/aped/state.yaml` from the umbrella tip and verify:
...
```bash
git show "${UMBRELLA}:$docs/aped/state.yaml" | yq '...'
```
