...
| "Let me sketch the deepened interface in the candidate's source file" | No edits to source files. Sketches go in the audit report at `docs/aped/architecture-audit.md`. |
...
| Output | `docs/aped/architecture.md` (greenfield arch decisions) | `docs/aped/architecture-audit.md` (ranked deepening candidates) |
...
1. **APED architecture artefact.** Read `docs/aped/architecture.md` if it exists — note any pattern decisions the audit must respect. If the candidate would contradict an existing decision, mark it clearly in the report (e.g. _"contradicts architecture.md §2.2 — but worth reopening because…"_) and don't suggest contradicting decisions casually.
2. **Brownfield context.** Read `docs/aped/project-context.md` if it exists — extract the modules and integration points the existing codebase already documents.
...
Write the audit report to `docs/aped/architecture-audit.md` (NEVER `architecture.md` — that's `aped-arch`'s artefact). Structure:
...
Update `docs/aped/state.yaml` if applicable: log the audit timestamp under `arch_audits.last_run` so future invocations know when the codebase was last reviewed.

## Handoff

Tell the user:

> *"Audit complete. {{N}} candidates accepted, {{M}} rejected, {{K}} deferred. Report at `docs/aped/architecture-audit.md`. To act on a candidate: invoke `aped-story` with the candidate's name to draft the implementation story; the story will pull the candidate's solution sketch + dependency category from the report."*
...
8. Output: `docs/aped/architecture-audit.md` lists `session.ts` as the single accepted candidate with the Phase 5 hybrid recommendation; `jwt.ts` and `permissions.ts` listed in "Rejected by deletion test".
9. Handoff: *"Audit complete. 1 candidate accepted (`Session` module deepening). Report at `docs/aped/architecture-audit.md`. Invoke `aped-story` with 'Session module deepening' to draft the implementation story."*

## Next Step

Tell the user: *"Audit shipped to `docs/aped/architecture-audit.md`. Invoke `aped-story` per accepted candidate, when ready. `aped-arch-audit` does not auto-chain."*
