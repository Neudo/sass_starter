...
- Respect `sprint.parallel_limit` and `sprint.review_limit`. **Schema v3 (6.1.0+) reads them from `.aped/config.yaml.sprint.*`; v2 scaffolds keep the legacy values under `state.yaml.sprint.*`.** Use the resolution snippet below — config.yaml wins, state.yaml is the v2 fallback, hardcoded defaults (`3` / `2`) are the last resort:
  ...
  if [[ -z "$PARALLEL_LIMIT" || "$PARALLEL_LIMIT" == "null" ]]; then
    PARALLEL_LIMIT=$(yq '.sprint.parallel_limit // 3' docs/aped/state.yaml)
  fi
  REVIEW_LIMIT=$(yq '.sprint.review_limit // ""' .aped/config.yaml)
  if [[ -z "$REVIEW_LIMIT" || "$REVIEW_LIMIT" == "null" ]]; then
    REVIEW_LIMIT=$(yq '.sprint.review_limit // 2' docs/aped/state.yaml)
  fi
  ```
...
3. Read `docs/aped/state.yaml` — must have `current_phase: "sprint"` and `sprint.stories` populated by `aped-epics`.
4. Read `docs/aped/epics.md` — for the DAG and story metadata.
...
Read `sprint.mode` from `.aped/config.yaml` (or `docs/aped/state.yaml`). Two modes:
...
```bash
EPIC_N=$(yq '.sprint.active_epic' docs/aped/state.yaml)
...
bash .aped/scripts/log.sh dispatch_started \\
  epic="$(yq '.sprint.active_epic' docs/aped/state.yaml)" \\
...
UMBRELLA=$(yq '.sprint.umbrella_branch' docs/aped/state.yaml)
...
**If a git worktree already exists for the story** (user ran `sprint-dispatch.sh` earlier, or aped-sprint was interrupted), use the recovery path. `sprint-dispatch.sh` writes the marker idempotently via the same helper, so the marker is guaranteed before the close+open cycle hands the pane to claude:

```bash
# 1. Ensure the worktree + marker exist (sprint-dispatch.sh calls the helper)
...
**If the recovery path was used** (`sprint-dispatch.sh` + `close` + `open` + `send` instead of fresh `workmux add`), add this line to the user instructions:
