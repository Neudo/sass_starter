...
Build and maintain `docs/aped/glossary.md` — the canonical place where every domain term used across PRD, architecture, stories, and code is defined once. Eliminates the "different word, same concept" drift that costs every team tokens and clarity.
...
3. Existing glossary at `docs/aped/glossary.md` — if present, treat as authoritative starting point.
4. Upstream APED artefacts that exist:
   - `docs/aped/prd.md` — most term-rich source (FRs, NFRs, user roles).
   - `docs/aped/architecture.md` — technical concepts.
   - `docs/aped/ux/**` — UI / interaction terms.
   - `docs/aped/product-brief.md` — business / market terms.
   - `docs/aped/project-context.md` (brownfield) — existing-system terms.
   - `docs/aped/stories/*.md` — story-specific vocabulary that may have drifted.
...
Glob `docs/aped/**`. Load every artefact found. Read existing `glossary.md` if it exists. Report to the user:
...
Once the user has walked the candidate list (or stopped early with `[A]bandon`), write `docs/aped/glossary.md` using the format below. Append + revise the existing file; never rewrite from scratch.
...
```bash
yq -r '."Avoid"[]' docs/aped/glossary.md | while read avoid; do
