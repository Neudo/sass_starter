...
2. Re-run validation: `bash .aped/aped-prd/scripts/validate-prd.sh docs/aped/prd.md`
3. Update epics: add/archive affected stories
4. Re-run coverage: `bash .aped/aped-epics/scripts/validate-coverage.sh docs/aped/epics.md docs/aped/prd.md`
5. Update `docs/aped/state.yaml`: mark affected stories as `backlog`
...
2. Archive current artifacts to `docs/aped/archive/{date}/`
...
1. Invalidate any now-stale epic-context caches — delete `docs/aped/epic-*-context.md` for the affected epic(s) so `aped-dev` recompiles on the next story.
2. Set `sprint.scope_change_active: false` in state.yaml (atomic).
3. Post a follow-up comment on each previously notified ticket:
   > "Scope change applied. If you're in an active worktree, pull the latest `docs/aped/` artefacts and restart your story loop — the epic-context cache has been invalidated."
