...
- Lessons — `docs/aped/lessons.md` (filter `Scope: aped-story` or `Scope: all`)
- Previous stories — `docs/aped/stories/*.md` from completed stories of the current epic
...
The cache at `docs/aped/epics-context/epic-{N}-context.md` is **fresh** when:
- It exists, AND
...
If absent or stale → compile by writing `docs/aped/epics-context/epic-{N}-context.md` with this exact shape (template strict — downstream consumers parse the headers):
...
**After writing or refreshing the cache (WARN-only in 6.3.0)** — run `bash .aped/scripts/validate-epic-context.sh docs/aped/epics-context/epic-{N}-context.md`. On non-zero, surface the stderr verbatim to the user, advise them to re-run `aped-story` (the cache is engine-owned; manual fixes risk going stale on the next refresh), and continue this step (the cache exists, just non-conformant). If the validator script is absent, warn-once and continue. Escalates to ERROR in 7.0.0 — at that point a non-conformant cache will block aped-review's append.
