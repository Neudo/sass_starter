...
> Story file ready at `docs/aped/stories/{story-key}.md` on branch `{branch-name}`. Run `aped-dev {story-key}` to implement.
...
```bash
git add docs/aped/stories/{story-key}.md docs/aped/state.yaml
