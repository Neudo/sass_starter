...
Write the story file at `docs/aped/stories/{story-key}.md` from the validated draft.
...
2. **Fallback** (MCP unavailable): edit `docs/aped/state.yaml` directly — set `sprint.stories.{key}.status: ready-for-dev`.
...
✅ Story file at `docs/aped/stories/{story-key}.md`.
