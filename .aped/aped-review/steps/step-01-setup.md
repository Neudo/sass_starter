...
- ✱ **Story file** — `docs/aped/stories/{story-key}.md`. Missing → *"No story file. Run `aped-story` first."*
- ✱ **Epic-context cache** — `docs/aped/epics-context/epic-{N}-context.md` (N = epic number from story key). Missing → *"No epic context cache. Run `aped-story` first — never work around with raw PRD/UX loads, that's the drift the cache exists to prevent."*

## Soft inputs (load if present)

- `docs/aped/architecture.md` — full file. Patterns are LAW. Missing → WARN, Code auditor runs in degraded mode.
- `docs/aped/lessons.md` — filter `Scope: aped-review` or `Scope: all`.
- Last `done` story of the same epic — `docs/aped/stories/{epic}-*.md` with status `done` (most recent). Skip if first story of epic.
