...
- Body: short, links to the Review Record at `docs/aped/stories/{story-key}.md`. Closes `{ticket}`.

The block must be **idempotent** — probe before create, edit if the PR already exists:

```bash
UMBRELLA=$(yq '.sprint.umbrella_branch' ${project_root}/docs/aped/state.yaml)
HEAD_BRANCH=$(git symbolic-ref --short HEAD)
TITLE="feat({ticket}): {story-key} — {short-title}"
BODY="Closes {ticket}. Review Record in docs/aped/stories/{story-key}.md."
...
Open `docs/aped/stories/{story-key}.md`, append after the existing `## Dev Agent Record`:
...
2. **Fallback** — edit `docs/aped/state.yaml`: story → `done` (all closed) or stays `review`.
