...
2. **Latest done artefact.** Glob `docs/aped/stories/*.md` and `docs/aped/quick-specs/*.md` — surface the most recent `done` artefact (the thing that just shipped). Read its title + AC.
3. **Active sprint check.** Read `docs/aped/state.yaml`. If `sprint.stories.*` contains any `in-progress | review-queued | review`, HALT per Disposition above.
