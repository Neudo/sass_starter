...
step: 2
reads:
  - "docs/aped/stories/{story-key}.md"
  - "docs/aped/epics-context/epic-{N}-context.md"
  - "docs/aped/architecture.md"
  - "docs/aped/lessons.md"
  - ".aped/**"
writes: []
mutates_state: false
---
...
- ✱ Story file — `docs/aped/stories/{story-key}.md`.
- ✱ Epic context cache — `docs/aped/epics-context/epic-{N}-context.md` (N = epic number from story key, e.g. `1-2-jwt` → epic 1).
- Architecture — `docs/aped/architecture.md` (full load — patterns are LAW for dev; architecture stays primordial, not in the cache).
- Lessons — `docs/aped/lessons.md` (filter `Scope: aped-dev` or `Scope: all` — produced by `aped-retro` after each epic).
- Last done story of same epic — the most recent `docs/aped/stories/{epic}-*.md` with status `done` (continuity for fine-grain decisions; skip if first story of epic).
...
For ✱ Story file:
- Found → continue.
- Missing → HALT: *"No story file found at `docs/aped/stories/{story-key}.md`. Run `aped-story` first to prepare it."*

For ✱ Epic context cache:
- Found → continue.
- Missing → HALT: *"No epic context cache found at `docs/aped/epics-context/epic-{N}-context.md`. Run `aped-story` first — it compiles the cache so `aped-dev` can stay token-light. Do not work around this by loading PRD/UX/architecture raw — that's exactly the drift the cache exists to prevent."*
