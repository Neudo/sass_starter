...
1. Check for existing PRFAQ: `docs/aped/prfaq.md`
...
> Scan `docs/aped/`, `docs/aped/brainstorm/`, and any user-provided paths for documents relevant to this concept. Return the 3-5 most relevant findings with 2-line summaries.
...
Create `docs/aped/prfaq.md` with frontmatter:
```yaml
---
stage: 1
...
Append the refined press release to `docs/aped/prfaq.md`. Update frontmatter: `stage: 2`, `updated: {date}`. Append `<!-- coaching-notes-stage-2 --> {rejected framings, competitive positioning, differentiators explored, out-of-scope details} <!-- / -->`.
...
Update `docs/aped/prfaq.md`: `stage: 3`. Append coaching notes.
...
Update `docs/aped/prfaq.md`: `stage: 4`. Append coaching notes.
...
Produce a concise verdict section at the end of `docs/aped/prfaq.md`:
...
- [ ] **Placeholder lint** — run `bash .aped/scripts/lint-placeholders.sh docs/aped/prfaq.md`.
...
PRFAQ is optional upstream tooling — it does NOT update `docs/aped/state.yaml` phases directly. But if the verdict is STRONG, tell the user:
> "PRFAQ complete (`docs/aped/prfaq.md`). Verdict: STRONG. When you're ready, run `aped-analyze` — the PRD distillate at the bottom of the PRFAQ is ready to seed it."
