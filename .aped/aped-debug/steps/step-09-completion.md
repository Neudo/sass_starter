...
- [ ] **Root cause documented** — the step 06 cause statement (`file:line`, condition, behaviour) is recorded in the commit message; if the bug represents a class worth remembering, also append to `docs/aped/lessons.md`.
...
- [ ] **Lint clean** — if the debug session produced a notes file (e.g. `docs/aped/debug/<bug>-{date}.md`), run `bash .aped/scripts/lint-placeholders.sh <notes-file>`.
