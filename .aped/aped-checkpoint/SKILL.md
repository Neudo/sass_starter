...
3. **State changes**: Read `docs/aped/state.yaml` — what phase/story moved? **If state.yaml is absent** (greenfield mid-flight), report "no state.yaml — pre-pipeline checkpoint" and continue with git-only inputs; do not invent a phase.
4. **New artifacts**: Check for new files in `docs/aped/` (specs, stories, reports)
