...
- Persist all lessons to `docs/aped/lessons.md` — retros must be actionable across epics
...
1. Read `docs/aped/state.yaml` — find the target epic:
...
> You are **Mia**, a systemic analyst reading post-mortem data. Read every story file in `docs/aped/stories/` with prefix `{epic-number}-`. Extract:
...
> You are **Ava**, a continuity auditor. Check if `docs/aped/retros/epic-{N-1}-retro.md` exists.
> - If yes: extract every action item, lesson, and team agreement from the previous retro
> - Cross-reference against `docs/aped/stories/` for epic {N} — was each action item applied? What's the evidence?
...
Read `docs/aped/epics.md` (or `docs/aped/epics/epic-{N+1}.md` if sharded) to surface the next epic's outline.
...
- [ ] **Placeholder lint** — run `bash .aped/scripts/lint-placeholders.sh docs/aped/retros/epic-{N}-retro-{date}.md`.
...
Write `docs/aped/retros/epic-{N}-retro-{date}.md`:
...
```bash
mkdir -p docs/aped/retros
```

### 2. Lessons file

Append distilled lessons to `docs/aped/lessons.md`:
  ...
  phases:
    sprint:
      ...
      retros:
        epic_{N}:
          status: done
          file: docs/aped/retros/epic-{N}-retro-{date}.md
...
> "Retro saved at `docs/aped/retros/epic-{N}-retro-{date}.md`. Lessons appended to `docs/aped/lessons.md`.
>
