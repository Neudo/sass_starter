...
- [ ] **Placeholder lint** — run `bash .aped/scripts/lint-placeholders.sh docs/aped/project-context.md`.
...
Write project context to `docs/aped/project-context.md`:
...
Update `docs/aped/state.yaml` under `pipeline.phases.context` with the structured fields below. The block is the canonical record of *which* kind of project this is — downstream skills read `type` to decide whether to apply brownfield bias.
