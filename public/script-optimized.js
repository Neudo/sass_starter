(function () {
  ...
  function setupClickHandler(selector, callback) {
    document.addEventListener("click", (e) => {
      try {
        let target = e.target.matches?.(selector)
          ? e.target
  ...
  function setupCustomListeners(domain, sessionId) {
    ...
    const clickEvents = customEvents.filter(
      (e) => e.event_type === "click" && e.event_selector
    );
      ...
      if (!globalClickListenerAdded) {
        ...
        document.addEventListener("click", (e) => {
          clickEventsConfig.forEach(({ event, domain, sessionId }) => {
            try {
              let target = e.target.matches?.(event.event_selector)
                ? e.target
  ...
  function startHeartbeat() {
    ...
    heartbeatInterval = setInterval(() => {
      if (Date.now() - lastActivityTime > 18e5) {
        // 30 min
        stopHeartbeat();
      } else {
        sendHeartbeat();
