...
export async function GET(request: NextRequest) {
  try {
    ...
    const csvFiles = await generateAllCsvs(
      adminClient,
      siteData.id,
      siteData.domain,
      startDate,
    ...
    const filename = `${domain.replace(
      ".",
      "_"
    )}_analytics_export_${startDate.replace(/-/g, "")}_${endDate.replace(
      /-/g,
      ""
    )}.zip`;
...
async function generateAllCsvs(
  ...
  const dateRange = `${startDate.replace(/-/g, "")}_${endDate.replace(
    /-/g,
    ""
  )}`;
  ...
  files.push({
    ...
    content: await generateLocationsCsv(
      adminClient,
      siteId,
      startDate,
      endDate
  ...
  files.push({
    ...
    content: await generatePagesCsv(
      adminClient,
      siteId,
      domain,
      startDate,
...
async function generateVisitorsCsv(
  adminClient: any,
  siteId: string,
  startDate: string,
  endDate: string
  ...
  sessions?.forEach((session: any) => {
    ...
    const duration = Math.floor(
      (lastSeen.getTime() - created.getTime()) / 1000
    );
...
async function generateLocationsCsv(
  adminClient: any,
  siteId: string,
  startDate: string,
  endDate: string
...
async function generateBrowsersCsv(
  adminClient: any,
  siteId: string,
  startDate: string,
  endDate: string
...
async function generateSourcesCsv(
  ...
  const { data: sessions } = await adminClient
    .from("sessions")
    .select(
      "created_at, referrer, utm_source, utm_medium, utm_campaign, utm_content, utm_term, page_views, last_seen"
    )
    .eq("site_id", siteId)
    .gte("created_at", startDate)
    .lte("created_at", endDate + "T23:59:59")
...
async function generatePagesCsv(
  adminClient: any,
  siteId: string,
  domain: string,
  startDate: string,
...
async function generateAllDataCsv(
  adminClient: any,
  siteId: string,
  startDate: string,
  endDate: string
  ...
  sessions?.forEach((session: any) => {
    ...
    csv += `"${date}","${session.id}","${session.country || ""}","${
      session.region || ""
      ...
      session.utm_medium || ""
    }","${session.utm_campaign || ""}","${session.utm_term || ""}","${
      session.utm_content || ""
    }","${session.channel || ""}"\n`;
  });
