  ...
  try {
    ...
    const funnelsWithAnalytics = await Promise.all(
      (funnels || []).map(async (funnel) => {
        const steps = (funnel.funnel_steps || []).sort((a, b) => a.step_number - b.step_number);
        ...
        const stepsWithData = steps.map(step => {
          ...
          const uniqueVisitors = new Set(stepConversions.map(c => c.session_id)).size;
          
          // Calculate conversion rate relative to first step
          const firstStepVisitors = steps.length > 0 ? 
            new Set(conversions?.filter(c => c.step_number === steps[0].step_number).map(c => c.session_id)).size : 0;
          const conversionRate = firstStepVisitors > 0 ? (uniqueVisitors / firstStepVisitors) * 100 : 0;
        ...
        const completedVisitors = stepsWithData[stepsWithData.length - 1]?.visitors || 0;
        const overallConversionRate = totalVisitors > 0 ? (completedVisitors / totalVisitors) * 100 : 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!siteId || !name || !steps || !Array.isArray(steps) || steps.length === 0) {
      return NextResponse.json(
