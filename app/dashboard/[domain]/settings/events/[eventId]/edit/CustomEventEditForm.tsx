...
interface CustomEventEditFormProps {
  siteId: string;
  domain: string;
  customEvent: CustomEvent;
}

export function CustomEventEditForm({
  siteId,
  domain,
  customEvent,
}: CustomEventEditFormProps) {
  const [formData, setFormData] = useState({
    name: customEvent.name,
    description: customEvent.description || "",
    event_type: customEvent.event_type,
    event_selector: customEvent.event_selector || "",
    trigger_config: customEvent.trigger_config || {},
    is_active: customEvent.is_active,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/custom-events/${customEvent.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to update custom event");
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Edit Custom Event</h1>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update your custom event settings to track specific user
            interactions
          </CardDescription>
        </CardHeader>
        <CardContent>
